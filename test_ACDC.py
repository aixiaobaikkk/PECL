...
parser.add_argument('--exp', type=str, default='MCCP', help='experiment_name')
parser.add_argument('--model', type=str, default='unet', help='model_name')
parser.add_argument('--num_classes', type=int,  default=4, help='output channel of network')
parser.add_argument('--labelnum', type=int, default=7, help='labeled data')
parser.add_argument('--stage_name', type=str, default='self_train', help='self or pre')
parser.add_argument('--model_path', type=str, default=r'unet_best_model.pth', help='self or pre')
...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            out_main = net(input)
            if len(out_main)>1:
                out_main=out_main[0]+out_main[1]+out_main[2]+out_main[3]
...
def Inference(FLAGS):
    ...
    snapshot_path = "./model/MCCP/ACDC_{}_{}_labeled/{}".format(FLAGS.exp, FLAGS.labelnum, FLAGS.stage_name)
    test_save_path = "./model/MCCP/ACDC_{}_{}_labeled/{}_predictions/".format(FLAGS.exp, FLAGS.labelnum, FLAGS.model)
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = net_factory_2d(net_type=FLAGS.model, in_chns=1, class_num=FLAGS.num_classes)
    save_model_path = os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))
    # path = r"E:\MCCP\code\model\MCCP\ACDC_MCCP_7_labeled\self_train\unet_best_model_0.9025523315496294.pth"
    # state = torch.load(str(path))
    # net.load_state_dict(state['net'])
    # net = net_factory_2d(net_type='pvt_cascade', in_chns=1, class_num=4, mode="train")
    # #
    save_model_path = FLAGS.model_path
...
def Inference_test(net):
    with open('../data_split/ACDC/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0] for item in image_list])
    first_total = 0.0
    second_total = 0.0
    third_total = 0.0
    test_save_path = "./model/MCCP/ACDC_MCCP_7_labeled/_predictions/"
