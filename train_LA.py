"""
DICE->89.62:8 labeled

DICE->88.02:4 labeled
"""
...
from utils.MCCP_utils import context_mask, mix_loss, parameter_sharing, update_ema_variables, sup_loss

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='../data_split/LA', help='Name of Dataset')
parser.add_argument('--exp', type=str, default='MCCP', help='exp_name')
...
parser.add_argument('--self_max_iteration', type=int, default=9000, help='maximum self-train iteration to train') # 9000
...
parser.add_argument('--workers', type=int, default=4, help='GPU to use')
...
def pre_train(args,model_name, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 200 == 0 and iter_num > int(pre_max_iterations/ 2):
                model.eval()
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                # 找到每个位置最常见的预测值
                most_common_values, _ = torch.mode(all_predictions, dim=4)

                # 将结果赋值给vote_prediction_a_unet3d
                vote_prediction_unet3d = most_common_values
                ...
                # 将结果赋值给vote_prediction_a_unet3d
                vote_prediction_voxresunet = most_common_values
                ...
                img_mask, loss_mask = context_mask(img, args.mask_ratio)

            mixl_img = img * img_mask + unimg * (1 - img_mask)
            mixu_img = img * (1 - img_mask) + unimg * img_mask

            # print(img_lab.shape, img_mask.shape, unimg_lab.shape,"oadjoqp")

            mixu_lab = img_lab * (1 - img_mask) + unimg_lab * img_mask
            mixl_lab = img_lab * img_mask + unimg_lab * (1 - img_mask)

            # grid_mask = Gridmask.GridMask_3D()
            # grid_mask.set_prob(iter_num, self_max_iterations)
            ...
            loss12 = 2 * loss_mse(vnet_predict.float(), unet3d_predict.float()) * get_current_consistency_weight(iter_num)
            loss13 = 2 * loss_mse(vnet_predict.float(), voxresunet_predict.float()) * get_current_consistency_weight(iter_num)            

            #loss12 =2 * loss_mse(vnet_predict.float(), unet3d_predict.float()) 
            #loss13 =2 * loss_mse(vnet_predict.float(), voxresunet_predict.float()) 
            ...
            if iter_num % 200 == 0:
                logging.info('iteration %d : loss: %03f  loss_l: %03f  loss_u: %03f  loss_mix: %03f best_dice : %04f' % (iter_num, loss,loss_l,loss_u,loss_mix, best_dice))
            ...
            if iter_num % 200 == 0 and iter_num > int( args.self_max_iteration/2):
                model1.eval()
...
def run_main(exp, labelnum):
    ...
    ## make logger file
    pre_snapshot_path1 = "./model/MCCP/LA_{}_{}_labeled/exp_{}/pre_train_model1".format(args.exp, args.labelnum, exp)
    pre_snapshot_path2 = "./model/MCCP/LA_{}_{}_labeled/exp_{}/pre_train_model2".format(args.exp, args.labelnum, exp)
    pre_snapshot_path3 = "./model/MCCP/LA_{}_{}_labeled/exp_{}/pre_train_model3".format(args.exp, args.labelnum, exp)
    self_snapshot_path = "./model/MCCP/LA_{}_{}_labeled/exp_{}/self_train".format(args.exp, args.labelnum, exp)
