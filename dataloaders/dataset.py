...
class Pancreas(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        # 文件路径
        self.file_paths = {
            'train': self._base_dir / 'train.txt',
            'test': self._base_dir / 'test.txt'
        }

        # 读取对应分割的文件列表
        self.image_list = self._read_file(self.file_paths[split])
    ...
    def _read_file(self, file_path):
        with open(file_path, 'r') as file:
            return [line.strip().replace('image', '') for line in file]
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        file_path = self._base_dir / 'data' / f'{image_name}'
        # file_path = self._base_dir / 'pancreas' / f'PANCREAS_{image_name}.h5'
...
if __name__ == "__main__":
    ...
    patch_size = (4, 96, 96)
    db_train = Pancreas(base_dir='../../data_split/Pancreas',
                        split='train',
    ...
    batch_sampler = TwoStreamBatchSampler(labeled_idxs, unlabeled_idxs, 12 , 6)
    sub_bs = int(6 / 2)
    ...
    iterator = tqdm(range(3000), ncols=70)
    import time

    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            volume_batch, label_batch = sampled_batch['image'][:6], sampled_batch['label'][
                                                                                  :6]
