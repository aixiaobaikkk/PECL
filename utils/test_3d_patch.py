...
# def var_all_case_PA(model, num_classes, patch_size=(96,96,96), stride_xy=18, stride_z=4):
#     with open('../data_split/Pancreas/test.txt', 'r') as f:
#         image_list = f.readlines()
#     image_list = ["../data_split/Pancreas/pancreas/" + "PANCREAS_" + item.replace('\n', '').replace('image','')  for item in
#                   image_list]
#     loader = tqdm(image_list)
#     total_dice = 0.0
#     for image_path in loader:
...
def var_all_case_PA(model, num_classes, patch_size=(96,96,96), stride_xy=18, stride_z=4):
    with open('../data_split/Pancreas/new/test.txt', 'r') as f:
        image_list = f.readlines()
    image_list = ["../data_split/Pancreas/new/data/" + item.replace('\n', '')  for item in
                  image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
...
def var_all_case_LA(model, num_classes, patch_size=(120, 120, 80), stride_xy=18, stride_z=4):
    with open('../data_split/LA/test.list', 'r') as f:
        image_list = f.readlines()
