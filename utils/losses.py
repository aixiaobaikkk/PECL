...
class mask_DiceLoss(nn.Module):
    ...
    def prob_forward(self, pred, target, mask=None):
        ...
        # N x C x H x W
        pred_one_hot = pred.view(N, nclass, -1).cuda()
        target = target.view(N, 1, -1).cuda()
        target_one_hot = to_one_hot(target.type(torch.long), nclass).type(torch.float32).cuda()
...
def update_model_ema(model, ema_model, alpha=0.99):
    for ema_param, param in zip(ema_model.parameters(), model.parameters()):
...
dice_loss = mask_DiceLoss(nclass=4)
CE = nn.CrossEntropyLoss(reduction='none')

def sup_loss(output, label,ce_weight=0.7,dice_weight=0.3):
    label = label.type(torch.int64)
    loss_dice = dice_loss(output, label)
    loss_ce = torch.mean(CE(output, label))
    loss = dice_weight * loss_dice + loss_ce * ce_weight
    return loss
